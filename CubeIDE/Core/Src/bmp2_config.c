...
BMP2_HandleTypeDef hbmp2[] = {
    {
      .SPI = &hspi1,
	  .CS_Port = BMP2_CS_GPIO_Port,
	  .CS_Pin = BMP2_CS_Pin,
      .MaxRetry = 10
    }
    // Usunięto pozostałe dwa wpisy, bo masz tylko jeden czujnik
};

/* Public variables ----------------------------------------------------------*/
struct bmp2_dev bmp2dev[BMP2_NUM_OF_SENSORS] = {
    BMP2_INIT_HANDLE(hbmp2[0]) // Zostaw tylko jeden
};
